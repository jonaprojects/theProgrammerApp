...
images to GitHub Container Registry (GHCR). Deployment is deliberately disabled
until a production environment is configured.
...
3. Copy `deploy/.env.production.example` to `.env.production` in that directory
   and replace every placeholder. The real file must never enter Git.
...
5. Keep `PRODUCTION_DEPLOY_ENABLED` unset while validating the first image
   publication. Set it to `true` when the host and DNS/TLS routes are ready.

## Release and rollback

Merging to `main` runs the entire pipeline. The `production` concurrency lock
prevents overlapping deployments, while the environment can require approval.
The public readiness checks must succeed before GitHub records a successful
release.
